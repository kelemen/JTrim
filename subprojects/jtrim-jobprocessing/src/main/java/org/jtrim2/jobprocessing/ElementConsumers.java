...
final class ElementConsumers {
    ...
    private static <T> SeqConsumer<T> toSingleShotSeqConsumer(SeqConsumer<? super T> seqConsumer) {
        ...
        return (cancelToken, seqProducer) -> {
            if (!started.compareAndSet(false, true)) {
                throw new IllegalStateException("This consumer does not support parallel processing.");
            }
    ...
    public static <T> SeqConsumer<T> concatSeqConsumers(
        ...
        return (cancelToken, producer) -> {
            seqConsumer1.consumeAll(cancelToken, ElementProducers.peekedSeqProducer(producer, seqConsumer2));
        };
    }
    ...
    public static <T, C extends Iterable<? extends T>> SeqConsumer<C> flatteningSeqConsumer(
            SeqConsumer<? super T> srcSeqConsumer) {

        Objects.requireNonNull(srcSeqConsumer, "srcSeqConsumer");

        if (srcSeqConsumer == drainingSeqConsumer()) {
            return drainingSeqConsumer();
        }

        return (cancelToken, seqProducer) -> {
            srcSeqConsumer.consumeAll(cancelToken, ElementProducers.flatteningSeqProducer(seqProducer));
        };
    }
